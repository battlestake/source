...
{
    return GetBoolArg("-testnet", false) ? 19543 : 9543;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop BattleStake server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "BattleStake server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: BattleStake-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("BattleStake-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use BattleStaked";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("BattleStake-rpchand");
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "getpowblocks"           && n > 0) ConvertTo<int64_t>(params[0]);
    if (strMethod == "getpowblocksleft"       && n > 0) ConvertTo<int64_t>(params[0]);
    if (strMethod == "getpowtimeleft"         && n > 1) ConvertTo<int64_t>(params[1]);
